...
struct node * bst_rotate_right(struct node *r)
{
	struct node *ltree;
	if(r==NULL)
		return r;
	if(!bst_isred(r->lchild))
		return r;
	ltree=r->lchild;
	r->lchild=ltree->rchild;
	r->size=1+sizeoftree(r->lchild)+sizeoftree(r->rchild);
	ltree->rchild=r;
	ltree->size=1+sizeoftree(ltree->lchild)+sizeoftree(ltree->rchild);
	ltree->color=r->color;
	r->color='R';
	return ltree;
}
struct node * bst_rotate_left(struct node *r)
{
	struct node *rtree,*ltree;
	if(r==NULL)
		return r;
	if(!bst_isred(r->rchild))
		return r;
	rtree=r->rchild;
	r->rchild=rtree->lchild;
	r->size=1+sizeoftree(r->lchild)+sizeoftree(r->rchild);
	rtree->lchild=r;
	rtree->size=1+sizeoftree(rtree->lchild)+sizeoftree(rtree->rchild);
	rtree->color=r->color;
	r->color='R';
	return rtree;
}
