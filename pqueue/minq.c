...
int minq_isempty(struct pqueue *pq)
{
	if(pq->noitem==0)
		return 1;
	return 0;
}
int minq_init(struct pqueue *pq,int size)
{
	pq->size=0;
	pq->noitem=0;
...
void minq_heapifydown(struct pqueue *pq, int pos)
{
	int lchild,rchild,j;
	struct pqitem temp;
	...
	if(pq->q[j].key < pq->q[pos].key)
	{
		...
		pq->q[j]=pq->q[pos];
		pq->q[pos] = temp;
		minq_heapifydown(pq,j); 
	}
	
}
void minq_heapifyup(struct pqueue *pq,long pos)
{
	int parent;
	struct pqitem temp;
	if(pos>0)
	{
		...
		if(pq->q[parent].key > pq->q[pos].key)
		{
			...
			pq->q[parent]=temp;
			minq_heapifyup(pq,parent);
		}
	}
}
int minq_insert(struct pqueue *pq,int key)
{
	int parent;
	...
	pq->noitem++;
	minq_heapifyup(pq,pq->noitem-1);
	return 1;	
}
void minq_delete(struct pqueue *pq,int pos)
{
	...
	if(pq->noitem >1)
	{
		...
		{
			minq_heapifyup(pq,pos);	
		}
		else
		{
			minq_heapifydown(pq,pos);
		}
	}
	else if (pq->noitem==1)
	{
...
struct pqitem * minq_extractmin(struct pqueue *pq)
{
	struct pqitem *min;
...
int main()
{
	...
	struct pqueue pq;
	if(!minq_init(&pq,sizeof(arr)/sizeof(arr[0])) )
	{
		exit(1);	
	}
	...
	for(i=0;i<sizeof(arr)/sizeof(arr[0]);i++)
	{
		printf("%d ",minq_extractmin(&pq)->key);	
	}
	printf("\n");
}
